        ...
        for (int i = 0; i < MAX_EXAMS; i++) {
            ...
            if (exam != NULL) {
                ...
                for (int j = 0; j < exam->question_count; j++) {
                    ...
                    write(socket, exam->questions[j].question, strlen(exam->questions[j].question));
                    for (int k = 0; k < 3; k++) {
                        printf("Enviando opción %c: %s\n", 'A' + k, exam->questions[j].options[k]);
                        write(socket, exam->questions[j].options[k], strlen(exam->questions[j].options[k]));
                    }
                    printf("Opción correcta: %c\n", 'A' + exam->questions[j].correct_option - 1);
                    write(socket, &exam->questions[j].correct_option, sizeof(int)); // Enviar la opción correcta
