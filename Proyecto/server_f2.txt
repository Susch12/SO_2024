...
void escribirPreguntasEnArchivos(Pregunta preguntas[], int total_preguntas) {
    FILE *archivoMatematicas = fopen("preguntas_matematicas.txt", "w");
    FILE *archivoEspanol = fopen("preguntas_espanol.txt", "w");
    FILE *archivoIngles = fopen("preguntas_ingles.txt", "w");

    if (!archivoMatematicas || !archivoEspanol || !archivoIngles) {
        perror("Error al abrir archivos para escribir");
        exit(1);
    }

    for (int i = 0; i < total_preguntas; i++) {
        if (strcmp(preguntas[i].categoria, "Matemáticas") == 0) {
            fprintf(archivoMatematicas, "%s|%s|%s| A) %s| B) %s| C) %s|%c\n",
                    preguntas[i].categoria, preguntas[i].numero, preguntas[i].pregunta,
                    preguntas[i].opcionA, preguntas[i].opcionB, preguntas[i].opcionC,
                    preguntas[i].respuesta_correcta);
        } else if (strcmp(preguntas[i].categoria, "Español") == 0) {
            fprintf(archivoEspanol, "%s|%s|%s| A) %s| B) %s| C) %s|%c\n",
                    preguntas[i].categoria, preguntas[i].numero, preguntas[i].pregunta,
                    preguntas[i].opcionA, preguntas[i].opcionB, preguntas[i].opcionC,
                    preguntas[i].respuesta_correcta);
        } else if (strcmp(preguntas[i].categoria, "Inglés") == 0) {
            fprintf(archivoIngles, "%s|%s|%s| A) %s| B) %s| C) %s|%c\n",
                    preguntas[i].categoria, preguntas[i].numero, preguntas[i].pregunta,
                    preguntas[i].opcionA, preguntas[i].opcionB, preguntas[i].opcionC,
                    preguntas[i].respuesta_correcta);
        }
    ...
    fclose(archivoMatematicas);
    fclose(archivoEspanol);
...
int cargar_preguntas(Pregunta preguntas[]) {
    ...
    fclose(file);
    escribirPreguntasEnArchivos(preguntas, total_preguntas);
...
void manejar_cliente(int nuevo_socket) {
    ...
    examen[valread - 1] = '\0';
    examen[strcspn(examen, "\r\n")] = 0; // Elimina caracteres de nueva línea

// Imprimir para depuración
    printf("Entrada recibida: %s\n", examen);

// Compara la entrada con opciones numéricas
    if (strcmp(examen, "1") == 0) {
      strcpy(examen, "Matemáticas");
    } else if (strcmp(examen, "2") == 0) {
      strcpy(examen, "Español");
    } else if (strcmp(examen, "3") == 0) {
