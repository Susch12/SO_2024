...
void handle_client(int client_socket) {
    ...
    // Debug: Imprimir matrícula y contraseña recibidas
    ...
    if (authenticate(matricula, password)) {
        ...
        if (exam != NULL) {
            for (int i = 0; i < exam->question_count; i++) {
                write(client_socket, exam->questions[i].question, strlen(exam->questions[i].question));
                write(client_socket, "\n", 1);
                for (int j = 0; j < 3; j++) {
                    write(client_socket, exam->questions[i].options[j], strlen(exam->questions[i].options[j]));
                    write(client_socket, "\n", 1);
                }
                char correct_option[2] = {exam->questions[i].correct_option, '\0'};
                write(client_socket, correct_option, 1);
                write(client_socket, "\n", 1);
            }
        }
    } else {
